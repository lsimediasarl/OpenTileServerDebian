...
#
# Simple tile server installation script with tilestache as backend
# for Debian Stretch (start with barbone install with ssh server only).
#
# This script will install open street map data and prepare the tilestache
# server to render the tiles directly via apache2
#
# The database will be stored under the defined user ("osm" by default) and the
# tiles will be stored in the /var/lib/tilestache
#
# The script must be run as root
#
# Usage: ./opentileserver_stache.sh {pbf_url}"
#
# Example
# ./opentileserver_stache.sh http://download.geofabrik.de/north-america/us/delaware-latest.osm.pbf
# ./opentileserver_stache.sh http://download.geofabrik.de/europe/switzerland-latest.osm.pbf
# ./opentileserver_stache.sh https://planet.openstreetmap.org/pbf/planet-latest.osm.pbf
#
# Licence
#
